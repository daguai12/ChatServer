...
int main(int argc,char* argv[])
{
  if(argc < 3)
  {
    cerr << "command invlid! example: ./ChatClient 127.0.0.1 6000" << endl;
    exit(-1);
  }
  ...
  for(;;)
  {
    ...
    case 2: //register业务
    {
      ...
      if(len == -1)
      {
          cerr << "send reg reponse error" << endl;
      }
      else
      {
        char buffer[1024] = {0};
...
void mainMenu(int clientfd)
{
  ...
  while(isMainMenuRunning)
  {
    ...
    //调用相应命令的事件处理回掉，mainMenu对修改封闭，添加新功能不需要修改该函数
...
//"chat" command handler
void chat(int clienfd,string str)
{
  int idx = str.find(":"); 
  if(-1 == idx)
  {
